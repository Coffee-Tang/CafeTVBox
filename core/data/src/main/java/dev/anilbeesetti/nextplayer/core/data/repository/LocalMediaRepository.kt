...
class LocalMediaRepository @Inject constructor(
    ...
    override fun observeFolders(folderPath: String?): Flow<List<Folder>> = mediaService.observeFolders(folderPath).map { mediaFolders ->
        mediaFolders.map { it.toFolder() }
    }

    override fun observeVideos(folderPath: String?): Flow<List<Video>> =
        combine(mediaService.observeVideos(folderPath), mediumStateDao.getAll()) { mediaVideos, mediumStates ->
    ...
    override suspend fun fetchFolders(folderPath: String?): List<Folder> = mediaService.fetchFolders(folderPath).map { it.toFolder() }

    override suspend fun fetchVideos(folderPath: String?): List<Video> = mediaService.fetchVideos(folderPath).mapAsync { mediaVideo ->
